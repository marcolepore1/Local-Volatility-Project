...
P1 = discount_factor*mean(max(S(2,:) - perc_strike(1)*S(1,:),0))
P2 = discount_factor*mean(max(S(2,:) - perc_strike(2)*S(1,:),0))

fwd(1) = interp1(T,Fwd,expiry(1));
fwd(2) = interp1(T,Fwd,expiry(2));
model_impl_vol1 = blsimpv(fwd(2),perc_strike(1)*fwd(1),0,expiry(2)-expiry(1),P1/discount_factor)
model_impl_vol2 = blsimpv(fwd(2),perc_strike(2)*fwd(1),0,expiry(2)-expiry(1),P2/discount_factor)
